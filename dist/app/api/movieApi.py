...
class movieAPI:
    ...
    def create_movie_model(self, movieResponse):
        ...
        self.dataJson += self.createValuesJson(json.loads(movieJson), Constants.movieKeys)
        if movie_model.torrents:
            torrents = movie_model.torrents
            for t_model in torrents:
                self.torrentJson += self.createValuesJson(json.loads(jsonEncoder().encode(t_model)),
                                                          Constants.torrentsKeys)
